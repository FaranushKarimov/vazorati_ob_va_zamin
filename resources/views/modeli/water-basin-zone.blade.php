      ...
      <div class="box">
        ...
        <div class="box-body table-responsive">
          <table class="table table-bordered">
            ...
            @foreach ($water_basin_zones as $water_basin_zone)
            <tr class="item-data-row">
              <td>
                <div class="btn-group-vertical">
                  <button class="btn btn-danger" data-toggle="modal" data-target="#deleteItemModal" onclick="deleteItem({{ $water_basin_zone->id }})"><i class="fas fa-trash"></i></button>
                  <a href="{{ route('modeli.water-basin-zone.edit',$water_basin_zone->id) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                </div>
              </td>
              <td class="h4">{{ $water_basin_zone->id }}</td>
              <td>{{ $water_basin_zone->name_ru }}</td>
              <td>{{ $water_basin_zone->name_tj }}</td>
              <td>{{ $water_basin_zone->name_en }}</td>
              <td>{{ $water_basin_zone->woc }}</td>
              <td>{{ $water_basin_zone->area }}</td>
              <td>{{ $water_basin_zone->created_at }}</td>
              <td>{{ $water_basin_zone->updated_at }}</td>
            </tr>
            @endforeach
          </table>
        </div>
  ...
  <div class="modal fade" id="addNewItemModal" tabindex="-1" role="dialog" aria-labelledby="addNewItemModalLabel">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        <form id="addItemForm" role="form" method="POST" action="{{ route('modeli.water-basin-zone.store') }}" accept-charset="UTF-8">
          @csrf
...
@section('js')
    ...
    function deleteItem(id)
     {
         var id = id;
         var url = '{{ route("modeli.water-basin-zone.destroy", ":id") }}';
