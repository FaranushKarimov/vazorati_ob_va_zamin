    ...
    <div class="col-md-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Водосборные площади</h3>
          <br>
          <div class="box-tools">
            {{ $catchment_areas->links() }}
          </div>
          <br>
        ...
        <div class="box-body table-responsive">
          <table class="table table-bordered">
            ...
            @foreach ($catchment_areas as $catchment_area)
            <tr class="item-data-row">
              <td>
                <div class="btn-group-vertical">
                  <button class="btn btn-danger" data-toggle="modal" data-target="#deleteItemModal" onclick="deleteItem({{ $catchment_area->id }})"><i class="fas fa-trash"></i></button>
                  <a href="{{ route('modeli.catchment-area.edit',$catchment_area->id) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                </div>
              </td>
              <td class="h4">{{ $catchment_area->id }}</td>
              <td>{{ $catchment_area->basin ? $catchment_area->basin->name_ru : $catchment_area->basin_id }}</td>
              <td>{{ $catchment_area->name_ru }}</td>
              <td>{{ $catchment_area->name_tj }}</td>
              <td>{{ $catchment_area->name_en }}</td>
              <td>{{ $catchment_area->woc }}</td>
              <td>{{ $catchment_area->district }}</td>
              <td>{{ $catchment_area->region }}</td>
              <td>{{ $catchment_area->republic }}</td>
              <td>{{ $catchment_area->area }}</td>
              <td>{{ $catchment_area->created_at }}</td>
              <td>{{ $catchment_area->updated_at }}</td>
            </tr>
            @endforeach
          </table>
        </div>
  ...
  <div class="modal fade" id="addNewItemModal" tabindex="-1" role="dialog" aria-labelledby="addNewItemModalLabel">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        <form id="addItemForm" role="form" method="POST" action="{{ route('modeli.catchment-area.store') }}" accept-charset="UTF-8">
          @csrf
...
@section('js')
    ...
    function deleteItem(id)
     {
         var id = id;
         var url = '{{ route("modeli.catchment-area.destroy", ":id") }}';
