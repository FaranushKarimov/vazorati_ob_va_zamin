    ...
    <div class="col-md-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Запрос водопользования</h3>
          <br>
          <div class="box-tools">
            {{ $qrequests->links() }}
          </div>
          ...
          <table class="table table-bordered">
            <thead>
              <tr>
                ...
                <th>{{ $columns["qms_request_per_day"] }}</th>
                <th>{{ $columns["created_at"] }}</th>
                <th>{{ $columns["updated_at"] }}</th>
              </tr>
            </thead>
            @foreach ($qrequests as $qrequest)
            <tr class="item-data-row">
              <td>
                <div class="btn-group-vertical">
                  <button class="btn btn-danger" data-toggle="modal" data-target="#deleteItemModal" onclick="deleteItem({{ $qrequest->id }})"><i class="fas fa-trash"></i></button>
                  <a href="{{ route('modeli.qrequest.edit',$qrequest->id) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                </div>
              </td>
              <td class="h4">{{ $qrequest->id }}</td>
              <td>{{ $qrequest->region_id }}</td>
              <td>{{ $qrequest->wua_id }}</td>
              <td>{{ $qrequest->qms_request_per_day }}</td>
              <td>{{ $qrequest->created_at }}</td>
              <td>{{ $qrequest->updated_at }}</td>
            </tr>
            @endforeach
          </table>
        </div>
  ...
  <div class="modal fade" id="addNewItemModal" tabindex="-1" role="dialog" aria-labelledby="addNewItemModalLabel">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        <form id="addItemForm" role="form" method="POST" action="{{ route('modeli.qrequest.store') }}" accept-charset="UTF-8">
          @csrf
          <div class="modal-body">
            <div class="box-body">
              ...
              <div class="form-group">
                <label for="add_qms_request_per_day">{{ $columns["qms_request_per_day"] }}</label>
                <input type="text" class="form-control" id="add_qms_request_per_day" name="qms_request_per_day" placeholder='{{ $columns["qms_request_per_day"] }}'>
              </div>
            </div>
          </div>
...
@section('js')
    ...
    function deleteItem(id)
     {
         var id = id;
         var url = '{{ route("modeli.qrequest.destroy", ":id") }}';
