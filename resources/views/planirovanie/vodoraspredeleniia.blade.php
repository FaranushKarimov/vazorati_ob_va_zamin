...
@section('content_header')
    <h1 id="printPageTitle">Балансовая ведомость</h1>
@stop
    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box box-primary box-solid">
                ...
                <div class="box-body">
                  <form id="filterForm" role="form" method="GET" action="{{ route('planirovanie.vodoraspredeleniia.index') }}" accept-charset="UTF-8">
                    <div class="row" id="filterRow1">
                        <div class="col-xs-4">
                          <div class="form-group">
                            <label for="name0"><span>Бассейн</span></label>
                        ...
                        <div class="col-xs-4">
                          <div class="form-group">
                            <label for="name0"><span>Магистральный Канал</span></label>
                        ...
                        <div class="col-xs-4">
                            <div class="form-group">
                              <label for="name1"><span>АВП</span></label>
            ...
            <table class="table table-bordered">
              <thead>
                <tr class="align-middle">
                  ...
                  <th>{{ $columns['water_vol_fact'] }}</th>
                  <th>{{ $columns['water_vol_diff'] }}</th>
                  <th>{{ $columns['date'] }}</th>
                </tr>
              </thead>
              ...
              <tr class="item-data-row">
                ...
                <td>{{ number_format($billing->water_vol/1000, 2) }}</td>
                <td>{{ number_format($billing->water_vol_fact, 2) }}</td>
                <td class="{{ (($billing->water_vol_fact - $billing->water_vol/1000) < 0) ? 'bg-danger' : 'bg-success' }}" >{{ number_format($billing->water_vol_fact - $billing->water_vol/1000, 2) }}</td>
  ...
  <script type="text/javascript">   
    $(document).ready(function(){
      ...
      @if(app('request')->input('canal_id'))
        $.ajax({
          url: "{{ route('planirovanie.vodoraspredeleniia_ajax_canal') }}",
          method: 'get',
          cache: false,
          data: {
      ...
      @if(app('request')->input('wua_id'))
        $.ajax({
          url: "{{ route('planirovanie.vodoraspredeleniia_ajax_wua') }}",
          method: 'get',
          cache: false,
          data: {
